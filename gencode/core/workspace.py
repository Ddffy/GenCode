...
class WorkspaceContext:
    ...
    def build(cls, cwd, repo_root_override=None):
        ...
        if repo_root_override is not None:
            ...
            # Temporary test/fixture workspaces must not inherit a dotfiles Git
            # repository from the user's home directory.
            try:
                temp_root = Path(tempfile.gettempdir()).resolve()
                cwd_in_temp = cwd.is_relative_to(temp_root)
                root_in_temp = discovered_root.is_relative_to(temp_root)
            except (AttributeError, OSError):
                cwd_in_temp = root_in_temp = False
            repo_root = cwd if cwd_in_temp and not root_in_temp else discovered_root
        docs = {}
